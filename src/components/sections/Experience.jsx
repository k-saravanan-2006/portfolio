...
export default function Experience() {
  ...
  return (
    <section id="experience" className="min-h-screen w-full relative overflow-hidden bg-black flex items-center justify-center py-20">
      ...
      <div className="container mx-auto px-6 sm:px-8 max-w-[1200px] w-full relative z-10">
        <h2
        ...
        <div className="relative border-l-0 md:border-l border-[rgba(216,236,248,0.2)] ml-0">
          {experiences.map((exp, idx) => (
            <div
              key={idx}
              className="mb-16 pl-0 md:pl-10 relative flex flex-col items-center md:items-start"
              data-aos="fade-up"
              data-aos-delay={idx * 150}
            >
              {/* Timeline dot — Hidden on very small mobile for a cleaner centered look, or repositioned */}
              <div className="hidden md:flex absolute left-[-21px] top-1 items-center justify-center w-10 h-10 rounded-full bg-[#000000] border-2 border-[var(--color-brand-glow)] shadow-[0_0_15px_rgba(216,236,248,0.4)]">
                <Briefcase size={20} weight="fill" className="text-[var(--color-brand-glow)]" />
              </div>

              <div className="w-full bg-[var(--glass-bg)] border border-[var(--glass-border)] backdrop-blur-[10px] p-8 rounded-2xl transition-all duration-300 hover:border-[rgba(216,236,248,0.3)] hover:shadow-[0_10px_30px_rgba(216,236,248,0.1)] text-center md:text-left">
